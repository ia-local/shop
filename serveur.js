...
async function generateCardContent(prompt) {
    try {
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                // MODIFICATION ICI : Instructions pour l'IA responsable dans la génération de contenu
        ...
        switch (mainAction) {
            ...
            case 'Mettre à jour les articles dynamiquement (Admin)':
                ...
                try {
                    ...
                    console.log('  > N\'oubliez pas de rafraîchir la page de la boutique sur le site web.');
                } catch (error) {
...
app.listen(PORT, async () => {
    ...
    console.log(`✨`);
    // L'ASCII Art principal est maintenant appelé au début de startInteractiveCLI
    console.log(`✨`);
    console.log(`✨ Server Telegram running ✨.Worker-ia_Pibot.`);
    console.log(`✨`);
    console.log(`✨ ----------------------------------------------------------->`);

    // Lancer l'interface CLI interactive après un court délai pour que les logs initiaux soient visibles
    if (process.env.TELEGRAM_BOT_TOKEN) {
        bot.launch().then(() => {
            console.log('Telegram bot launched! ✨ Worker-ia_Pibot.');
            setTimeout(startInteractiveCLI, 1000); // Lance la CLI après 1 seconde
        })
        .catch(err => {
            console.error('Failed to launch Telegram bot:', err);
            setTimeout(startInteractiveCLI, 1000); // Lance la CLI même si le bot Telegram échoue
        });
    } else {
        console.warn('TELEGRAM_BOT_TOKEN est pas configurer dans votre votre .env. Le bot Telegram ne sera pas lancé.');
        setTimeout(startInteractiveCLI, 1000); // Lance la CLI si le bot Telegram n'est pas configuré
    }
});
